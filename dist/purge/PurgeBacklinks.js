...
async function collectPages(api, listType, titleValue, verbose) {
    ...
    while (continueParam !== eol) {
        ...
        if (verbose) {
            console.log(`[${listType}] ${contParam}: ${continueParam === eol ? 'END' : String(continueParam)}, 已收集 ${result.length} 个页面`);
        }
...
async function runWithConcurrency(items, concurrency, worker, delayMs) {
    let success = 0;
    let fail = 0;
    let index = 0;
...
async function purgePages(api, pages, dryRun, verbose, concurrency) {
    const BATCH_SIZE = 50;
    const batches = [];
    for (let i = 0; i < pages.length; i += BATCH_SIZE) {
        batches.push(pages.slice(i, i + BATCH_SIZE));
    }
    const { success: batchSuccess, fail: batchFail } = await runWithConcurrency(batches, concurrency, async (batch) => {
...
async function nulleditPages(api, pages, dryRun, verbose, concurrency) {
    return runWithConcurrency(pages, concurrency, async (title) => {
        if (dryRun) {
