...
async function fetchFileList(api, pageTitle) {
...
async function clearPoemQueue(api, pageTitle) {
    const { data } = await api.post({
        action: 'query',
        prop: 'revisions',
        rvprop: 'content',
...
async function fetchDeletedRevisions(api, title) {
    const { data } = await api.post({
        action: 'query',
        list: 'deletedrevs',
        drtitle: title,
...
async function undoLastEdit(api, title, comment) {
    let revisions;
    try {
        const { data } = await api.post({
            action: 'query',
...
async function processFile(api, rawFilename, comment, options, index, total) {
