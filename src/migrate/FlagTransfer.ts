 ...
 */
function resolveFilename(suffix: string): string {
  // 已经是完整文件路径
  if (suffix.startsWith('File:')) return suffix;
  if (/\.\w+$/.test(suffix)) return `File:${suffix}`;
  return `File:Flag_of_${suffix}.svg`;
}

function buildFlagMap(): Map<string, string> {
  ...
  for (const line of lines) {
    ...
    // 跳过 {{{1|China}}} 这类默认值行
    if (leftPart.includes('{{{')) continue;
    // 跳过值为 None 的条目
    if (rightPart === 'None') continue;
...
function loadCompleted(): Set<string> {
  try {
    if (existsSync(COMPLETED_FILE)) {
      const data: CompletedRecord = JSON.parse(readFileSync(COMPLETED_FILE, 'utf-8'));
...
async function findFlagiconUsages(
  api: MediaWikiApi,
): Promise<Set<string>> {
  const usedParams = new Set<string>();
  const eol = Symbol();
  let eicontinue: string | symbol | undefined;
  ...
  while (eicontinue !== eol) {
    const { data } = await api.post({
      ...
      einamespace: '0',
      eilimit: 50,
      eicontinue: eicontinue as string | undefined,
    });

    const pages = (data as any).query.embeddedin as any[];
    const titles: string[] = pages.map((p: any) => p.title);
    ...
    for (let i = 0; i < titles.length; i += 50) {
      ...
      const pagesObj = (contentData.data as any).query.pages;
    ...
    eicontinue = (data as any).continue?.eicontinue ?? eol;
    if (eicontinue !== eol) {
...
async function fetchCommonsDirectUrl(suffix: string): Promise<string> {
  ...
  if (!pages) throw new Error('WMF API 响应无 pages');
...
function fetchCommonsFallbackUrl(suffix: string): string {
  const filename = resolveFilename(suffix);
  return `https://commons.wikimedia.org/wiki/Special:FilePath/${encodeURIComponent(filename.replace(/^File:/i, ''))}`;
}

async function getCommonsFileUrl(suffix: string): Promise<string> {
  try {
 ...
 * 检查文件是否已存在于萌娘共享站
 */
async function checkFileExistsOnCm(api: MediaWikiApi, filename: string): Promise<boolean> {
  try {
    const { data } = await api.post({
      action: 'query',
      prop: 'imageinfo',
...
async function main(): Promise<void> {
  ...
  if (args.verbose) console.log('[详细模式]');
  ...
  // 4. 加载已完成记录
  const completed = args.reset ? new Set<string>() : loadCompleted();
  if (args.reset) console.log('已重置完成记录');

  // 5. 筛选待处理
  const pending: Array<{ name: string; suffix: string }> = [];
  for (const name of usedParams) {
    const suffix = flagMap.get(name) || name;
    if (completed.has(suffix)) {
      if (args.verbose) console.log(`  跳过已完成: ${name} -> ${suffix}`);
      continue;
    }
